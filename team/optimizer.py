...
def other_P(P,theta,v_w,theta_w,k):
    # 其他队员的功率
    ratio = 0.4
    return v(P,theta,v_w,theta_w,k) * (m * g * math.sin(theta) + mu * m * g - k * ratio * v(P,theta,v_w,theta_w,k) * math.cos(theta_w))
...
def W_change_team(W_team,P,x,theta,v_w,theta_w,mem_no,k):
    for i in range(np.size(W_team)):
        if i == mem_no:
            # 如果是顶风的队员
            W_team[i] = func(float(W_s[i]),float(W_team[i]),P,CP[i],x,theta,v_w,theta_w,k) 
        else:
            P_other = other_P(P,theta,v_w,theta_w,k)
            if P_other > CP[i]: 
                W_team[i] -= (P_other-CP) * x / v(P_other,theta,v_w,theta_w,k)
            else:
                W_exp = W_s[i] - W_team[i] 
                t = float(x) / v(P_other,theta,v_w,theta_w,k) 
                W_team[i] = W_s[i] - W_exp * math.pow(math.e,-t/tau(P_other)) 
    # 返回第一个人的剩余能量
...
def T_total(P,turn_choice):
    ...
    for i in range(np.size(P)):
        ...
        if W_first_test < 0: 
            # 如果所有人都能量不足，集体进入休息
            P[i] = 0.99 * CP[mem_no]   
            W_change_team(W_bal[mem_no],float(P[i]),float(x[i]),float(theta[i]),float(v_w[i]),float(theta_w[i]),mem_no,k)
        elif W_first_test < change_ratio * W_s[i] :
            # 如果能量较小，则换人
            mem_no = (mem_no + 1) % 6
            W_change_team(W_bal[mem_no],float(P[i]),float(x[i]),float(theta[i]),float(v_w[i]),float(theta_w[i]),mem_no,k)
        else:
